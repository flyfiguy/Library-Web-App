...
export const Loans = ()  => {
    ...
    useEffect(()=>{
        const fetchUserCurrentLoans = async () => {
            if(authState && authState.isAuthenticated) {
                ...
                if(!shelfCurrentLoansResponse.ok){
                    throw new Error('Something went wrong in fetchUserCurrentLoans');
                }
        ...
        //Each time useEffect is called, scroll to top of page
        window.scrollTo(0,0);
    }, [authState, checkout]);
    ...
    //If there is an error, return the error to the UI
    if(httpError) {
        return(
    ...
    async function returnBook(bookId: number) {
        ...
        if(!returnResponse.ok){
            throw new Error('Something went wrong in returnBook fetch');
        }
    ...
    async function renewLoan(bookId: number) {
        ...
        const returnResponse = await fetch(url,requestOptions);
        if(!returnResponse.ok) {
            throw new Error("Something went wrong in renewLoan fetch");
        }
        setCheckout(!checkout);
    }
                        ...
                        {shelfCurrentLoans.map(shelfCurrentLoan => (
                            <div key={shelfCurrentLoan.book.id}>
                                <div className="row mt-3 mb-3">
                                    <div className="col-4 col-md-4 contianer">
