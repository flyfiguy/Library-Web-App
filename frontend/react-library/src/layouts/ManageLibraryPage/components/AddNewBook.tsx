...
export const AddNewBook = () => {
    ...
    //Convert image to base 64 before sending to the backend
    ...
    function getBase64 (file: any) {
        //FileReader() lets web application asynchronously read the contents of files stored on users computer
        let reader = new FileReader(); 

        //Change object into base 64
        reader.readAsDataURL(file);

        //Set image as a piece of state        
        reader.onload = function() {
            setSelectedImage(reader.result);
        };

        //Set the FileReader onerror function to log the error
    ...
    async function submitNewBook() {
        ...
        if(authState?.isAuthenticated && title !== '' && author !== '' && catagory !== 'Catagory'
            && description !== '' && copies >= 0) {
                const book: AddBookRequest = new AddBookRequest(title, author, description, copies, catagory);
                ...
                //Throw new error if request failed
                if(!submitNewBookResponse.ok) {
                    throw new Error('AddNewBook.submitNewBook: Something went wrong.');
                }

                //If Request was successful, set state values back to default
                setTitle('');
                setAuthor('');
                setDescription('');
                setCopies(0);
